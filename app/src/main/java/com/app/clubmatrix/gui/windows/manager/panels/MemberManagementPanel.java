...
public class MemberManagementPanel extends JPanel {
    ...
    private void addMember(ActionEvent event) {
        AddMemberDialog addMemberDialog = new AddMemberDialog((JFrame) JFrame.getFrames()[0], memberService);
    ...
    private void editMember(ActionEvent event) {
        ...
        if (selectedRow >= 0) {
            Member selectedMember = ((MemberTable) memberTable.getModel()).getMemberAt(selectedRow);
            EditMemberDialog editMemberDialog = new EditMemberDialog((JFrame) JFrame.getFrames()[0], memberService, selectedMember);
            editMemberDialog.setVisible(true);
            loadMembers(); // Reload members to reflect changes
        } else {
    ...
    private void deleteMember(ActionEvent event) {
        ...
        if (selectedRow >= 0) {
            ...
            int confirm = JOptionPane.showConfirmDialog(
                    this,
                    "Are you sure you want to delete member: " + selectedMember.getName() + "?",
                    "Confirm Deletion",
