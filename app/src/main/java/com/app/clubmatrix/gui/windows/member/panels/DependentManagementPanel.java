...
public class DependentManagementPanel extends JPanel {
  ...
  public DependentManagementPanel(
  ...
  private void addDependent(ActionEvent event) {
    AddDependentDialog addDependentDialog = new AddDependentDialog(
  ...
  private void editDependent(ActionEvent event) {
    ...
    if (selectedRow >= 0) {
      Dependent selectedDependent =
        ((DependentTable) dependentTable.getModel()).getDependentAt(
            selectedRow
          );
      EditDependentDialog editDependentDialog = new EditDependentDialog(
        (JFrame) JFrame.getFrames()[0],
        dependentService,
        username,
        selectedDependent
      ...
      JOptionPane.showMessageDialog(
        this,
        "Please select a dependent to edit.",
  ...
  private void deleteDependent(ActionEvent event) {
    ...
    if (selectedRow >= 0) {
      ...
      JOptionPane.showMessageDialog(
        this,
        "Please select a dependent to delete.",
        "No Dependent Selected",
        JOptionPane.WARNING_MESSAGE
      );
    }
