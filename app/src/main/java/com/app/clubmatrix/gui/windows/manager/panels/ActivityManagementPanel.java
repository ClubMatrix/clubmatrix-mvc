...
public class ActivityManagementPanel extends JPanel {
    ...
    private void addActivity(ActionEvent event) {
        AddActivityDialog addActivityDialog = new AddActivityDialog((JFrame) JFrame.getFrames()[0], ActivityService);
    ...
    private void editActivity(ActionEvent event) {
        ...
        if (selectedRow >= 0) {
            Activity selectedActivity = ((ActivityTable) ActivityTable.getModel()).getActivityAt(selectedRow);
            EditActivityDialog editActivityDialog = new EditActivityDialog((JFrame) JFrame.getFrames()[0], ActivityService, selectedActivity);
            editActivityDialog.setVisible(true);
            loadActivities(); // Reload Activities to reflect changes
        } else {
            JOptionPane.showMessageDialog(this, "Please select a Activity to edit.", "No Activity Selected", JOptionPane.WARNING_MESSAGE);
        }
    }
    ...
    private void deleteActivity(ActionEvent event) {
        ...
        if (selectedRow >= 0) {
            ...
            JOptionPane.showMessageDialog(this, "Please select a Activity to delete.", "No Activity Selected", JOptionPane.WARNING_MESSAGE);
        }
    }
}